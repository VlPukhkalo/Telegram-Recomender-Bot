...
from sqlalchemy import Column, update, delete, dialects, literal_column
...
class BaseDAL:
    ...
    @classmethod
    def sync_add(cls, **kwargs) -> Optional[int]:
        try:
            db = Database.get_instance()

            with db._sync_session() as session:
                with session.begin():
                    model = cls._get_model()
                    query = insert(model).values(**kwargs)
    ...
    def sync_get(
        ...
        with db._sync_sessionsession() as session:
            with session.begin():
    ...
    @classmethod
    def sync_update(cls, id: Optional[int] = None, where: Optional[Dict] = None, **kwargs) -> None:
        db = Database.get_instance()
        model = cls._get_model()
    ...
    @classmethod
    def sync_delete(cls, id: Optional[int] = None, *args, **kwargs) -> None:
        db = Database.get_instance()
        model = cls._get_model()
    ...
    @classmethod
    def sync_insert_or_update(cls, set_: dict, index_elements: List[str], *rows, **kwargs):
        db = Database.get_instance()
    ...
    @classmethod
    def sync_adds(cls, *rows, returning_id: bool = False, **kwargs) -> Optional[int]:
        db = Database.get_instance()
